...
class ApplicationWindow(QMainWindow):
    ...
    def initUI(self):
        ...
        self.grid.addWidget(plot_canvas, 1, 0, 5, 10)
        self.grid.addWidget(self.textbox, 0, 11)
        self.grid.addWidget(self.comboBox, 1, 11)
    ...
    def updateBtns(self):
        ...
        for i in range(1, max_rows.astype(int) + 1):
            ...
            self.grid.addWidget(self.list_array[i - 1], i + 5, 0)

            for j in range(1, columns + 1):
                self.box_array.append(QCheckBox(self))
                self.grid.addWidget(self.box_array[numB], i+5, j)
                numB += 1

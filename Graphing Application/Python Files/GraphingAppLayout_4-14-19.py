...
class channel:
    ...
    def gap(self,y_gap):
        print(type (self.y))
    def filter(self, filter_num):
        mod_y = self.y.tolist()
        mod_y2 = [i for i in mod_y if i <= filter_num]
        print(mod_y2)
...
class ApplicationWindow(QMainWindow):
    ...
    def initUI(self):
        ...
        self.y_gapBox = QLineEdit(self)

        # create filter textbox
        self.filterLabel = QLabel('Filter', self)
        self.filterBox = QLineEdit(self)
        ...
        self.grid.addWidget(self.filterLabel, 0, 1)
        self.grid.addWidget(self.filterBox, 0, 2)
        self.grid.addWidget(self.yLabel, 1, 1)
        self.grid.addWidget(self.y_gapBox, 1, 2)
    ...
    def updatePlot(self):
        ...
        # intantiate y_gap value for later use using the current y_gapBox value
        y_gap = self.y_gapBox.text()
        y_filter = self.filterBox.text()

        # check that user has entered a y gap value
        if y_gap == "":
            QMessageBox.about(self,"Error", "Please enter a y-gap value.")
        if y_filter == "":
                QMessageBox.about(self, "Error", "Please enter a filter value.")
        else:

            for j in range(0, len(self.channels_selected)):
                print(self.channels_array[self.channels_selected[j]].y)
                self.channels_array[self.channels_selected[j]].filter(y_filter)
